...
- `hooks/` — `useContact.ts`, `useReveal.ts`. `scripts/fb-post.mjs` (Groq → FB tự động), `scripts/fb-page.mjs` (cập nhật thông tin Page + đăng/ghim bài giới thiệu — chạy tay, cần Page token có `pages_manage_metadata`+`pages_manage_posts` trong `.env.local`). `supabase/*.sql`. `docs/SRS.md`.
